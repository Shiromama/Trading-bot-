...
# Risk model
STOP_LOSS_ACCOUNT_FRACTION = 0.10   # risk 10% of current balance
...
# Dataset split assumption for sim
TRAIN_TEST_SPLIT = 0.85

BROKER_FEE_PER_TRADE = 0.0
STOP_IF_BALANCE_BELOW = 0.01

# Threshold sweep
THRESHOLD_VALUES = [0.45, 0.50, 0.55, 0.60, 0.65, 0.70]
...
TRADE_LOG_PATH = SCRIPT_DIR / "lgbm_simulation_trade_log.csv"
SUMMARY_PATH = SCRIPT_DIR / "lgbm_simulation_summary.txt"
EQUITY_PLOT_PATH = SCRIPT_DIR / "lgbm_simulation_equity_curve.png"
DRAWDOWN_PLOT_PATH = SCRIPT_DIR / "lgbm_simulation_drawdown_curve.png"
THRESHOLD_SWEEP_PATH = SCRIPT_DIR / "lgbm_simulation_threshold_sweep.csv"
...
def infer_price_series(df: pd.DataFrame, split_idx: int) -> pd.Series:
    """
    Tries to find a usable price series for notional sizing.
    Falls back to REFERENCE_PRICE if nothing exists.
    """
    candidate_cols = [
        "entry_price",
        "1m_close", "5m_close", "15m_close",
        "1m_open", "5m_open", "15m_open",
        "close", "open",
        "price"
    ]
...
def prepare_features(df: pd.DataFrame, feature_list: list) -> pd.DataFrame:
    """
    Rebuild model input from dataset using the exact saved model feature list.
    """
    exclude_cols = [
        "target",
        "future_return",
    ...
    extra_features = [c for c in X.columns if c not in feature_list]

    if missing_features:
        raise ValueError(
            "Dataset is missing features required by the trained model:\n"
            f"{missing_features}\n\n"
            "This usually means your dataset builder, training script, and sim script "
            "are not from the same feature version."
        )

    if extra_features:
        print(f"[INFO] Dataset has {len(extra_features)} extra numeric columns not used by the model.")

    X = X[feature_list].copy()
    return X


def prepare_prediction_frame(df: pd.DataFrame, model, feature_list: list, split_idx: int) -> pd.DataFrame:
...
def simulate_account(trades_df: pd.DataFrame):
    if trades_df.empty:
    ...
    for trade_number, row in enumerate(sim.itertuples(index=False), start=1):
        ...
        # Conservative logic using final horizon return only
    ...
    summary = {
        ...
        "avg_pnl": float(result_df["pnl"].mean()),
        "avg_return_per_trade": float(result_df["effective_raw_return"].mean()),
        ...
        "final_horizon_exits": int((result_df["exit_reason"] == "final_horizon").sum()),
...
def make_plots(trade_log: pd.DataFrame):
    ...
    plt.title("LightGBM Simulation Equity Curve")
    ...
    plt.xlabel("Trade Number")
    plt.ylabel("Drawdown %")
    plt.title("LightGBM Simulation Drawdown Curve")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def run_threshold_sweep(prob_df: pd.DataFrame):
    ...
    for threshold in THRESHOLD_VALUES:
        ...
        results.append({
            ...
            "final_horizon_exits": summary["final_horizon_exits"],
...
def main():
    ...
    lines = [
        ...
        f"Average PnL:      ${summary['avg_pnl']:.4f}",
        f"Avg return/trade: {summary['avg_return_per_trade']:.6f}",
        ...
        f"Final-horizon exits: {summary['final_horizon_exits']}",
        ...
        "",
        "Note:",
        "- This simulator uses final-horizon returns from the dataset.",
        "- SL/TP are applied as money-based caps on those returns.",
        "- It does not know the exact intrabar path unless your dataset includes it.",
        "",
        "========== THRESHOLD SWEEP ==========",
    ]
